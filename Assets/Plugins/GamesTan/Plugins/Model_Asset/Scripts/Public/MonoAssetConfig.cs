...
namespace RealDream {
    ...
    public partial class MonoAssetConfig : AssetConfig {
        ...
        public override void DoInit() {
#if UNITY_EDITOR
            cfgInfos.Clear();
            PathUtil.Walk(PathUtil.GetFullPath(GlobalProjectConfig.Instance.ResourcesDir + "Config"), "*.asset", (path) => {
            ...
            assetInfos.Clear();
            PathUtil.Walk(PathUtil.GetFullPath(GlobalProjectConfig.Instance.ResourcesDir + "Prefab"), "*.prefab|*.asset", (path) => {
                var strs = Path.GetFileName(path).Split('_');
                try {
                    if (int.TryParse(strs[0], out var id)) {
